...
/*
todo: 阅读此注解源码
JVM intrinsics are methods in the JDK for which the JITs emit specialized sequences of machine
instructions which can be directly inlined into the caller. For example on x86 Integer.bitCount(int)
can be replaced with the POPCNT instruction.
 ...
 * assembly and/or hand-written compiler IR(编译器中间表示) -- a compiler intrinsic (编译器内部函数：编译器生存的函数，不包含在库文件中) -- to improve
